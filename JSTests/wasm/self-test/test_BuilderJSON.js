...
(function SectionsWithSameCustomName() {
    ...
    assert.throws(() => b.Unknown("foo"), Error, `Cannot have to sections with the same name "foo" and ID 0`);
})();
...
(function TwoTypeSections() {
    const b = (new Builder()).Type().End();
    assert.throws(() => b.Type(), Error, `Cannot have to sections with the same name "Type" and ID 1`);
})();
...
(function ImportBeforeTypeSections() {
    const b = (new Builder()).Import().End();
    assert.throws(() => b.Type(), Error, `Bad section ordering: "Import" cannot precede "Type"`);
})();

(function ImportFunctionWithoutTypeSection() {
    const i = (new Builder()).Import();
    assert.throws(() => i.Function("foo", "bar", 0), Error, `Can't use type 0 if a type section isn't present`);
})();

(function ImportFunctionWithInvalidType() {
    const i = (new Builder()).Type().End().Import();
    assert.throws(() => i.Function("foo", "bar", 0), Error, `Type 0 doesn't exist in type section`);
})();
...
(function ExportFunctionWithoutTypeSection() {
    ...
    assert.throws(() => e.Function("foo", 0, 0), Error, `Can't use type 0 if a type section isn't present`);
})();

(function ExportFunctionWithInvalidType() {
    const e = (new Builder()).Type().End().Export();
    assert.throws(() => e.Function("foo", 0, 0), Error, `Type 0 doesn't exist in type section`);
})();
...
(function ExportMismatchedImport() {
    const e = (new Builder())
        ...
        .Import().Function("foo", "bar", { params: [] }).End()
        .Export();
    assert.throws(() => e.Function("foo", 0, { params: ["i32"] }), Error, `Re-exporting import "bar" as "foo" has mismatching type`);
})();
...
(function InvalidFunctionParameters() {
    for (let invalid of ["", "void", "bool", "any", "struct", 0, 3.14, undefined, [], {}]) {
        ...
        assert.throws(() => c.Function({ params: [invalid] }), Error, `Type parameter ${invalid} isn't a valid value type`);
    }
})();
...
(function ExportUndefinedFunction() {
    ...
    assert.throws(() => c.End(), Error, `Export section contains undefined function "foo"`);
})();
